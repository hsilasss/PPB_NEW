...
class _RewardStorePageState extends State<RewardStorePage> {
  ...
  Widget build(BuildContext context) {
    Widget body;
    if (currentIndex == 1) {
      body = _buildRewardContent(context);
    } else {
      body = Center(child: Text(currentIndex == 0 ? 'Home Page' : 'Profile Page'));
    }
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(child: Padding(padding: const EdgeInsets.symmetric(horizontal: 20), child: body)),
    ...
    return SafeArea(
      child: Column(
        children: [
          ...
          const Text('Reward store', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w500)),
        ],
      ),
    );
  }
